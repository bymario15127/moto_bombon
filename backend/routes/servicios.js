...
router.post("/", async (req, res) => {
  try {
    const { nombre, duracion, precio, descripcion, imagen, precio_bajo_cc, precio_alto_cc } = req.body;
    ...
    const result = await db.run(
      "INSERT INTO servicios (nombre, duracion, precio, descripcion, imagen, precio_bajo_cc, precio_alto_cc) VALUES (?, ?, ?, ?, ?, ?, ?)",
      [nombre, duracion, precio || null, descripcion || "", imagen || "/img/default.jpg", precio_bajo_cc || null, precio_alto_cc || null]
    );
...
router.put("/:id", async (req, res) => {
  try {
    ...
    const result = await db.run(
      "UPDATE servicios SET nombre = ?, duracion = ?, precio = ?, descripcion = ?, imagen = ?, precio_bajo_cc = ?, precio_alto_cc = ? WHERE id = ?",
      [nombre, duracion, precio || null, descripcion, imagen, precio_bajo_cc || null, precio_alto_cc || null, id]
    );
